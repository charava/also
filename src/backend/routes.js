  ...
  */
      router.post('/add-reply', (req, res) => {
        UserSchema.findById(req.body.id)
          .then(user => {
            user.replies.push({from_name: req.body.from_name, from_email: req.body.from_email, message: req.body.message})
            UserSchema.findByIdAndUpdate(req.body.id, {replies: user.replies})
              .catch(err => {
                console.log(req.body.email)
                console.error(err)
              })
            console.log('Added a reply to the note!')
            console.log(user)
            res.json(user)
          })
          .catch(err => {
...
router.put('/add-reply', (req, res) => {
  UserSchema.findByIdAndUpdate(req.body.id, {replies: {from_name: req.body.name, from_email: req.body.email, message: req.body.message}}, {returnDocument: 'after'})
    .then(user => {
      console.log('added the reply!')
      console.log(user)
      res.json(user)
    })
    .catch(err => {
